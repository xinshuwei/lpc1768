...
void BCMenuData::SetWideString(const wchar_t *szWideString)
{
	...
	if (szWideString)
    {
		...
		if (m_szMenuText)
		{
			temp_size=sizeof(wchar_t);
			wcscpy_s(m_szMenuText, temp_size*(wcslen(szWideString)+1), szWideString);
		}
    }
	else
	{
...
//	if(nFlags & MF_POPUP)m_AllSubMenus.Add((HMENU)nID);
	...
	//else if (nFlags&MF_STRING)  //MF_STRING is 0, unreachable code, zhb remarked
	//{
...
void BCMenu::InsertSpaces(void)
{
	...
	for(i=0;i<numitems;++i)
	{
		...
		if(j!=-1)
		{
			...
			{
				newstring+=_T(' ');//SK: modified for Unicode correctness
				LPCTSTR lpstrText2 = (LPCTSTR)(newstring);
				t=pDC->GetTextExtent(lpstrText,_tcslen(lpstrText2));
			}
...
void BCMenu::GetShadowBitmap(CBitmap &bmp)
{
	...
	if(!IsWinXPLuna())
	{
		shadowcol=LightenColor(shadowcol,0.49);
	}
...
BOOL BCMenu::CanDraw3DImageList(int offset)
{
	...
	if(offset+1<numcurrent&&offset+2<numcurrent)
	{
		...
		if(m_AllImagesID[offset+1]==nID&&m_AllImagesID[offset+2]==nID)
		{
				retflag=TRUE;
		}
	}
